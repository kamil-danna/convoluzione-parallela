...
int main(int argc, char *argv[])
{
    ...
    // Condividiamo le dimensioni dell'immagine con tutti i processi
    MPI_Bcast(&width, 1, MPI_INT, 0, MPI_COMM_WORLD);
    MPI_Bcast(&height, 1, MPI_INT, 0, MPI_COMM_WORLD);
    MPI_Bcast(&max_val, 1, MPI_INT, 0, MPI_COMM_WORLD);

    // Ogni processo si genera il suo kernel (più veloce che inviarlo)
    float *kernel = generate_gaussian_kernel(k_size);

    // Calcoliamo la distribuzione dei carichi di lavoro (Scatterv/Gatherv)
    int *sendcounts = NULL;
    int *displs = NULL;
    if (rank == 0)
    {
        ...
        for (int i = 0; i < size; i++)
        {
            ...
            sendcounts[i] = rows * width; // Quanti pixel inviare
            displs[i] = offset;           // Da dove iniziare
            offset += sendcounts[i];
        }
    }

    // Calcoliamo quante righe spettano a questo specifico processo
    ...
    // Copiamo i nostri veri dati al centro del buffer padded
    memcpy(padded_data + (r * width), local_data, local_rows * width);

    // Definiamo i vicini (MPI_PROC_NULL gestisce automaticamente i bordi senza far crashare il programma)
    int top_neighbor = (rank == 0) ? MPI_PROC_NULL : rank - 1;
    int bottom_neighbor = (rank == size - 1) ? MPI_PROC_NULL : rank + 1;

    // Scambio con il vicino SOPRA
    ...
    // Clamping manuale per i bordi esterni (Rank 0 clona in su, l'ultimo Rank clona in giù)
    if (rank == 0)
    {
        for (int i = 0; i < r; i++)
        ...
        for (int x = 0; x < width; x++)
        {
            ...
            for (int ky = -r; ky <= r; ky++)
            {
                for (int kx = -r; kx <= r; kx++)
                {
                    ...
                    int py = y + r + ky; // 'r' compensa l'offset dell'halo nel buffer padded
    ...
    if (rank == 0)
    {
        printf("Tempo di convoluzione MPI (%d processi): %.4f secondi\n", size, end_time - start_time);
