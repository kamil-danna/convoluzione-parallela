...
# ---------------------------------------------------------
cores = [1, 2, 4]
speedup_omp = [0.97, 2.24, 2.96]
speedup_mpi = [0.94, 1.78, 3.16]
ideal = [1, 2, 4]
...
plt.xticks(cores)
plt.legend()
plt.savefig('1_Speedup.png', dpi=300, bbox_inches='tight')
plt.close()

# ---------------------------------------------------------
# 2. Grafico Efficienza Parallela
# ---------------------------------------------------------
eff_omp = [0.97, 1.12, 0.74]
eff_mpi = [0.94, 0.89, 0.79]
...
plt.legend()
plt.savefig('2_Efficienza.png', dpi=300, bbox_inches='tight')
plt.close()
...
# Aggiunge i valori sopra le colonne
for bar in bars:
    yval = bar.get_height()
    plt.text(bar.get_x() + bar.get_width()/2, yval + 0.02, f"{yval:.2f}", ha='center', fontsize=10)

plt.savefig('3_Ibrido.png', dpi=300, bbox_inches='tight')
plt.close()
...
eff_kernel = [0.872, 0.824, 0.913, 0.997] # Inserito il dato corretto per il K=7
...
plt.savefig('4_Kernel.png', dpi=300, bbox_inches='tight')
plt.close()

print("I 4 grafici sono stati generati con successo in formato PNG ad alta risoluzione!")